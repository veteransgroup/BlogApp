...
class BlogExtrasTestCase(TestCase):
    def setUp(self):
        ...
        self.cate = Category.objects.create(name="test data")
        self.ctx = Context()
    ...
    def test_show_recent_posts_without_any_post(self):
        ...
        self.assertInHTML("No articles!", expected_html)
    ...
    def test_show_categories_without_any_category(self):
        ...
        expected_html = template.render(context)
        self.assertInHTML('<h3 class="widget-title">categories</h3>', expected_html)
        self.assertInHTML("no category", expected_html)

    def test_show_categories_with_categories(self):
        cate_with_posts = Category.objects.create(name="document categories")
        Post.objects.create(
            title="test title -1", body="test content", category=cate_with_posts, author=self.user,
        )
        another_cate_with_posts = Category.objects.create(name="another document categories")
        Post.objects.create(
            title="test title-2",
            body="test content",
    ...
    def test_show_tags_without_any_tag(self):
        ...
        expected_html = template.render(context)
        self.assertInHTML('<h3 class="widget-title">Tags cloud</h3>', expected_html)
        self.assertInHTML("no tags!", expected_html)
    ...
    def test_show_archives_without_any_post(self):
        ...
        expected_html = template.render(context)
        self.assertInHTML('<h3 class="widget-title">Archives</h3>', expected_html)
        self.assertInHTML("no archives", expected_html)
